...
const sourcePath = path.join(workspaceDir, "lib", "broad-product-pilot-content.ts");
const familyPath = path.join(workspaceDir, "config", "broad-product-pilot-families.json");
const productPath = path.join(workspaceDir, "config", "broad-product-pilot-amazon-products.json");
...
const guides = pilotModule.broadProductPilotGuides;
...
const legacyAsins = new Set(legacyProducts.map((product) => product.asin));
const legacyFamilyKeys = new Set(legacyProducts.map((product) => `${product.site}:${product.familySlug}`));
for (const product of productData.products) {
  const key = `${product.site}:${product.familySlug}`;
  if (legacyAsins.has(product.asin)) errors.push(`Pilot ASIN collides with legacy inventory: ${product.asin}`);
  if (legacyFamilyKeys.has(key)) errors.push(`Pilot family collides with legacy Amazon inventory: ${key}`);
...
for (const family of familyData.families) {
  if (new RegExp(`slug:\\s*[\"']${family.familySlug}[\"']`).test(legacyExpansionSource)) {
    errors.push(`Pilot family appears in the prior governed expansion: ${family.site}:${family.familySlug}`);
  }
}
...
const uniqueImages = new Set(guides.map((guide) => guide.image)).size;
if (uniqueImages !== guides.length) errors.push(`Expected one unique editorial image per guide; found ${uniqueImages} for ${guides.length} guides`);
...
const report = {
  ...
  uniqueEditorialImages: uniqueImages,
  coreWords: {
...
console.log("Broad product pilot quality audit");
...
console.log("\nBroad product pilot quality audit passed.");
