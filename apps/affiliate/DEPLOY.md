...
The public Costume launch began with 25 editor-reviewed product URLs. The owner-approved July 26 Halloween pilot adds 12 exact products without changing or deactivating any other purchase link. Preview it with `npm run publish:costume:halloween --workspace apps/affiliate`; append `-- --apply` only after all 12 rows pass exact title, current stock, authorized Feed image, dedicated PID/AID, embedded destination, and active-link checks. The apply step only upserts the selected Halloween editorial records and merchandising fields. It must never be substituted for the full-catalog link validator.

The home, `/halloween`, category, Premium, guide, and policy pages are indexable; `/catalog`, filter combinations, and every product outside an approved editorial cohort stay `noindex,follow` and out of the sitemap. Purchase availability is a separate gate: every in-stock catalog product may show a CTA after its official Feed link passes the dedicated PID/AID, exact embedded destination, authorized-image, and live merchant-destination checks. Run the production validator from the repository root with `node --env-file=apps/affiliate/.env apps/affiliate/scripts/activate-cj-catalog-links.mjs --network-direct --apply`. Exact-destination HTTP 429 responses are recorded as merchant rate limiting rather than product drift; redirects to the merchant homepage, 404 responses, mismatched variants, or tracking-identity failures remain inactive. Use `npm run publish:costume:cohort --workspace apps/affiliate` to preview any later general cohort change and append `-- --apply` only after the preview passes exact-link, image, availability, and category checks.
