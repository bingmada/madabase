...
本轮完成可执行的本地修改和核验，未发布服务器。完整线上验收尚未关闭：Google 日配额阻塞 23 个 URL 检查；Costume 数据访问失败；发布后效果与生产验证仍未发生。
...
2. **Costume**：现有本地数据库配置为 localhost:5432，应用目录查询失败；9 月 25 日公网 Santa 页返回 403，已有授权 CJ SFTP 读取又在握手时超时。**32 个目录依赖的分类 / 编辑页购买路径没有通过**，76 个商品的新发现入口和 2 个同名 Santa 商品的真实差异仍需数据访问恢复后核验。源码修复已做，未凭标题把商品合并或删除。
3. **服务器与效果**：未部署、未重启、未提交索引。历史服务器事件保留，恢复原因需真实日志确认。发布后仍须核验公开页面、资源、发现路径、QA 统计排除，然后在原定 9/25、10/4、10/7、10/8、10/11 闸门观察；正文变化会混杂原标题实验，记录变更而不重置日期。
4. **归因与收益**：CJ 收益不可读取，Amazon Other 尚未归因，Clarity 历史 QA 污染无法靠新版追溯清除。没有编造收益、改 tag 抢归因或声称流量已恢复。

## 发布包与代码

Affiliate / Wellness 两个最新包已经包含本轮修复。Main 沿用先前 4883d9c 的包。仅准备交付，不执行服务器发布。
