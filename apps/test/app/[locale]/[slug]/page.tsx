...
export default async function TestDetailPage({
  ...
  const canonicalUrl = buildAbsoluteUrl(`/${locale}/${slug}`);
  ...
  const startHref = `/${locale}/${slug}/start${ref ? `?ref=${encodeURIComponent(ref)}` : ""}`;
            ...
            <section className="border-t border-[var(--border)] p-5 sm:p-7">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {relatedTools.map((tool) => (
                  <a key={tool.slug} href={`${process.env.NEXT_PUBLIC_MAIN_SITE_URL ?? "https://madabase.com"}/${locale}/tools/${tool.slug}`} className="group rounded-md border border-[var(--border)] bg-white p-4 transition hover:-translate-y-0.5 hover:border-[var(--brand)] hover:shadow-[var(--shadow-soft)]">
