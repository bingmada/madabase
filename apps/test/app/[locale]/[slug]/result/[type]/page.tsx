...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string; type: string }> }): Promise<Metadata> {
  ...
  const metadata = buildPageMetadata({
    ...
    locale,
    path: `/${slug}/result/${type.toLowerCase()}`,
...
export default async function TestResultPage({
  ...
  const canonicalUrl = buildAbsoluteUrl(`/${locale}/${slug}/result/${type.toLowerCase()}`);
  const unlockCost = test.unlockCost;
  const shareLink = user ? await getOrCreateTestShareLink(user.id, slug) : null;
  const shareUrl = shareLink ? buildAbsoluteUrl(`/${locale}/${slug}?ref=${shareLink.code}`) : null;
  const posterTestUrl = shareUrl ?? buildAbsoluteUrl(`/${locale}/${slug}`);
  ...
  const currentResultPath = `/${locale}/${slug}/result/${type.toLowerCase()}${currentResultQuery ? `?${currentResultQuery}` : ""}`;
  ...
  const breadcrumbSchema = buildBreadcrumbSchema([
    ...
    { name: content.title, item: buildAbsoluteUrl(`/${locale}/${slug}`) },
  ...
  return (
    <div className="min-h-screen bg-transparent">
      <Header locale={locale} pathname={`/${slug}`} />
      <main className="page-shell">
        ...
        <article className="surface-card-strong overflow-hidden">
          <header className="border-b border-[var(--border)] p-5 sm:p-7">
            ...
            <Link href={`/${locale}/${slug}/start`} className="mt-6 inline-flex h-11 items-center rounded-md border border-[var(--border)] bg-white px-4 text-sm font-semibold text-[var(--text)] transition hover:border-[var(--brand)]">
              {copy.retake}
            </Link>
