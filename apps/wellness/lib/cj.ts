...
function parseCjTrackingUrl(value: string) {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
  ...
  if (
    url.protocol !== "https:" ||
    !cjTrackingDomains.some((domain) => matchesDomain(url.hostname, domain))
  ) {
    return null;
  }
...
function isAllowedDestination(value: string, allowedHosts: readonly string[]) {
  try {
...
export function verifiedCjRedirectUrl(input: {
  ...
  if (
    ...
    !isAllowedDestination(input.destinationUrl, input.allowedDestinationHosts)
  ) {
    return null;
  }
  ...
  if (
    embeddedDestination !== input.destinationUrl ||
    !isAllowedDestination(embeddedDestination, input.allowedDestinationHosts)
  ) {
    return null;
  }
...
export function newCjSid() {
  return `ww_${crypto.randomUUID().replaceAll("-", "")}`;
}
